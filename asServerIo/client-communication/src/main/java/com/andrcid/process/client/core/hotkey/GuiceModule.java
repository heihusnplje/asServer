...
public class GuiceModule extends AbstractModule {
	...
	private void scannerClass() {
		...
		Collection<Class<?>> types = ClassScannerFactory.getInstance().classScanner(scannerPackage).getTypesAnnotatedWith(Module.class);
		...
		if(types != null && !types.isEmpty()) {
			for (Class type : types) {
				...
				if(type.isAnnotationPresent(Controller.class)) {
					...
					Module m = (Module)type.getAnnotation(Module.class);
					Class<?> from = m.from();
					if(from != void.class) {//如果没有指定from，则获取此类的所有的接口，如果接口==1则直接使用此接口
