	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		if(msg != null) {
			...
			if(p.getMessageType() == MessageType.HeartBeat) {
				...
				LOGGER.info("read heartBeat from client {}", JsonHelper.toJson(message));
				
				Session session = SessionManager.getInstance().getSession(ctx);
				if(session == null) {//注册
			        messageProcessor.processReceiveConnect(ctx, message);
				} else {//保持连接
					messageProcessor.processReceivePing(ctx);
				}
				return;
			}
